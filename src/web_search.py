...
def _rss_results(url: str, channel: str) -> list[SearchResult]:
    ...
    for item in soup.find_all("item"):
        ...
        if title and link:
            results.append(SearchResult(title=BeautifulSoup(title, "html.parser").get_text(" ", strip=True), url=link, source=source, published=date or "", snippet=BeautifulSoup(description or "", "html.parser").get_text(" ", strip=True)[:300], channel=channel))
    return results
...
def _bing_html_results(query: str, channel: str = "Bing Web") -> list[SearchResult]:
    try:
        ...
        for card in soup.select("li.b_algo")[:10]:
            ...
            results.append(SearchResult(title=anchor.get_text(" ", strip=True), url=anchor["href"], source=urlsplit(anchor["href"]).netloc, published="", snippet=snippet_node.get_text(" ", strip=True) if snippet_node else "", channel=channel, searched_query=query))
        return results
    ...
    for _, source in sources.iterrows():
        ...
        for page_url in pages:
            ...
            for link in soup.find_all("a", href=True):
                ...
                results.append(SearchResult(title=title, url=article_url, source=str(source.get("name", base_url)), published="", snippet="نتيجة من الفهرس العام للمصدر", match=min(0.95, overlap / max(1, len(terms))), channel="مصدر يمني مباشر"))
    return results
...
def _tavily_results(query: str) -> list[SearchResult]:
    ...
    if not api_key or api_key.strip().lower() in {"مفتاح tavily الخاص بك", "your tavily key", "ضع-المفتاح-هنا"}:
        return []
    try:
        response = requests.post("https://api.tavily.com/search", json={"api_key": api_key, "query": query, "search_depth": "advanced", "max_results": 10, "include_answer": False, "include_raw_content": True}, timeout=30)
        response.raise_for_status()
        items = response.json().get("results", [])
        return [SearchResult(title=str(item.get("title", "")), url=str(item.get("url", "")), source=str(item.get("url", "")).split("/")[2] if item.get("url") else "ويب عام", published=str(item.get("published_date", "")), snippet=str(item.get("raw_content") or item.get("content", ""))[:10000], channel="فهرس ويب موسع", searched_query=query) for item in items if item.get("url")]
...
def _ai_search_queries(claim: str) -> list[str]:
    ...
    try:
        response = requests.post(f"{base}/chat/completions", headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}, json={"model": os.getenv("NEWSGUARD_SEARCH_MODEL", "gpt-4o-mini"), "temperature": 0, "response_format": {"type": "json_object"}, "messages": [{"role": "user", "content": prompt}]}, timeout=30)
        response.raise_for_status()
...
def _extract_sahaafa_publisher(soup: BeautifulSoup) -> str:
    """Extract the original publisher shown by Sahaafa, not Sahaafa itself."""
    try:
        marker = soup.find(string=lambda value: value and "المصدر" in value)
        if marker:
            parent = marker.parent
            link = parent.find_next("a", href=True)
            if link:
                name = link.get_text(" ", strip=True)
                if name and name != "صحافة نت":
                    return name
    except Exception:
        pass
    return ""
...
def _enrich_page(result: SearchResult) -> SearchResult:
    try:
        ...
        for node in soup(["script", "style", "noscript", "svg"]):
            node.decompose()
        if "sahaafa.net" in urlsplit(result.url).netloc.lower():
            publisher = _extract_sahaafa_publisher(soup)
            if publisher:
...
def _user_provided_results(query: str) -> tuple[str, list[SearchResult]]:
    ...
    for raw_url in dict.fromkeys(urls):
        ...
        result = SearchResult(title="رابط أدخله المستخدم", url=url, source=host, published="", snippet="", match=0.0, channel=channel, searched_query=clean_query.strip(), match_type="رابط مباشر مقدم من المستخدم")
        try:
...
def search_public_web(query: str, sources_path: Path, max_results: int = 25) -> list[dict]:
    ...
    tavily_queries = [query, *ai_variants[:3], f'"{query}"', f'site:sahaafa.net "{query}"', f'site:facebook.com "{query}"', f'site:x.com "{query}"', f'site:twitter.com "{query}"']
    ...
    for search_query in variants:
        ...
        feeds = [(f"https://news.google.com/rss/search?q={encoded}&hl=ar&gl=YE&ceid=YE:ar", "Google News"), (f"https://www.bing.com/news/search?q={encoded}&format=rss", "Bing News"), (f"https://news.google.com/rss/search?q={quote('site:sahaafa.net ' + search_query[:180])}&hl=ar&gl=YE&ceid=YE:ar", "صحافة نت عبر فهرس الأخبار"), (f"https://www.bing.com/news/search?q={quote('site:sahaafa.net ' + search_query[:180])}&format=rss", "صحافة نت عبر Bing"), (f"https://news.google.com/rss/search?q={quote('site:facebook.com ' + search_query[:180])}&hl=ar&gl=YE&ceid=YE:ar", "Facebook عام عبر فهرس الأخبار"), (f"https://news.google.com/rss/search?q={quote('site:x.com ' + search_query[:180])}&hl=ar&gl=YE&ceid=YE:ar", "X عام عبر فهرس الأخبار")]
