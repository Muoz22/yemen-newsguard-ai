...
m1.metric("سجلات مرجعية",len(df)); m2.metric("مصادر البحث",len(sources_df)); m3.metric("أنواع المحتوى","5"); m4.metric("الحالة","جاهز")
...
with tab_text:
    ...
    search_web=st.checkbox("البحث في الويب والأخبار العامة والمصادر اليمنية",value=True)
    max_results=st.slider("عدد النتائج القصوى",20,100,50,10)
    if st.button("ابحث وقارن وتتبع الانتشار",type="primary"):
...
with tab_image:
    ...
    if st.button("فحص الصورة",type="primary"):
...
with tab_video:
    ...
    if st.button("فحص الفيديو",type="primary"):
...
with tab_fb:
    ...
    if st.button("جلب وتحليل",type="primary"):
